...
/* USER CODE BEGIN PV */
#define LSM303AGR_I2C_ADDRESS  (0x1E << 1)
/* USER CODE END PV */
...
void send_raw(uint16_t packet_number, int16_t raw_temp) {
    uint16_t data[3];
    data[0] = 0xAAAB;
    data[1] = packet_number;
    data[2] = raw_temp;
    CDC_Transmit_FS((uint8_t*)data, 6);
}
/* USER CODE END 0 */
...
int main(void)
{
  ...
  while (1)
  {
	  int16_t temperature = read_temperature(&hi2c1);
	  send_raw(packet_number, temperature);
	  packet_number++;
	  HAL_Delay(1000);
    /* USER CODE END WHILE */
